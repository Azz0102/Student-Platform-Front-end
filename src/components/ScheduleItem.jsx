...
function convertToEvent(data, fromDate) {
    ...
    const { dayOfWeek, time, classSessionName, classroomName, numOfHour } = data;
...
export function ScheduleItem({ semester, setEdited,setListEventSaved,setMetadata,setDate }) {
    ...
    function onSubmit() {
        startUpdateTransition(async () => {
            const response = await axios.post(`https://${process.env.NEXT_PUBLIC_BASE_URL}/api/admin/scheduling`, data, {
                headers: {
            ...
            setListEventSaved(metadata.map((item)=>{
        ...
        <Credenza open={isOpen} onOpenChange={setIsOpen}>
            ...
            <CredenzaContent className="flex flex-col gap-6 sm:max-w-md">
                <CredenzaHeader className="text-left">
                    ...
                    <CredenzaTitle>Tải File</CredenzaTitle>
                </CredenzaHeader>
